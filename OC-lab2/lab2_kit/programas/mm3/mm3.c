...
#define SUB_MATRIX_SIZE (CACHE_LINE_SIZE / sizeof(int16_t))
    ...
    for (size_t i = 0; i < N; i += SUB_MATRIX_SIZE) {
        for (size_t j = 0; j < N; j += SUB_MATRIX_SIZE) {
            for (size_t k = 0; k < N; k += SUB_MATRIX_SIZE) {
                // NOTE: we are obtaining a pointer to the first element of a
                // submatrix inside our original matrices. However, we cannot
                // use double indexing (matrix[i][j]). We need to add N to the
                // pointer to drop to the lower line.
                //
                // Exercise: figure out why.
                int16_t const *block_1 = &factor1[i][k];
                int16_t const *block_2 = &factor1[k][j];
                int16_t *block_res = &res[i][j];

                for (size_t inner_i = 0; inner_i < SUB_MATRIX_SIZE;
                     ++inner_i, block_res += N, block_1 += N) {
                    for (size_t inner_k = 0; inner_k < SUB_MATRIX_SIZE;
                         ++inner_k, block_2 += N) {
                        for (size_t inner_j = 0; inner_j < SUB_MATRIX_SIZE;
                             ++inner_j) {
                            block_res[inner_j] +=
                                block_1[inner_k] * block_2[inner_j];
                        }
                    }
                }
            }
