...
int main() {
    ...
    for (size_t cache_size = CACHE_MIN; cache_size <= CACHE_MAX;
        ...
        for (size_t stride = 1; stride <= cache_size / 2; stride = stride * 2) {
            ...
            /************************************/
            for (size_t repeat = 0; repeat <= N_REPETITIONS * stride;
                 repeat++) {
            ...
            float const avg_time =
                (float)(end_usec - start_usec) / n_iterations;
            float const avg_cycles =
                (float)(end_cycles - start_cycles) / n_iterations;
            fprintf(stdout,
